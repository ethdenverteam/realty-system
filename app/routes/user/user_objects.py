"""
User objects routes
Логика: управление объектами недвижимости, публикация, превью
"""
...
user_objects_bp = Blueprint('user_objects', __name__)
logger = logging.getLogger(__name__)

@user_objects_bp.route('/dashboard/objects', methods=['GET'])
@jwt_required
...
@user_objects_bp.route('/dashboard/objects/list', methods=['GET'])
@jwt_required
...
@user_objects_bp.route('/dashboard/objects/create', methods=['GET'])
@jwt_required
...
@user_objects_bp.route('/dashboard/objects/<object_id>', methods=['GET'])
@jwt_required
...
@user_objects_bp.route('/dashboard/objects/publish', methods=['POST'])
@jwt_required
                ...
                if photos_json and len(photos_json) > 0:
                    ...
                    if isinstance(photo_path, str) and photo_path:
                        ...
                        if os.path.exists(full_path):
                            ...
                            if photo_file_id:
                                # Падаем обратно на отправку по file_id
                                send_url = f'{url}/sendPhoto'
                                payload = {
                                    'chat_id': telegram_chat_id,
                                    'photo': photo_file_id,
...
@user_objects_bp.route('/dashboard/objects/<object_id>/preview', methods=['POST'])
@jwt_required
def user_preview_object_in_bot(object_id, current_user):
    ...
    try:
        ...
        # Поддерживаем несколько форматов photos_json (аналогично публикации в чаты):
        # - строка с путем к файлу
        # - dict с 'path' (новый формат)
        # - dict только с 'file_id' (legacy)
        photos_json = obj.photos_json or []
        ...
        if photos_json and len(photos_json) > 0:
            ...
            if photo_path:
                ...
                # Нет пути, но есть legacy file_id – отправляем фото по нему
                url = f'https://api.telegram.org/bot{BOT_TOKEN}/sendPhoto'
                payload = {
                    'chat_id': int(current_user.telegram_id),
                    'photo': photo_file_id,
